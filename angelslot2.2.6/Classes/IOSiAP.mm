...
{
    NSLog(@"productsRequest APProductsRequestDelegate");
    // release old
    ...
    //int index1 = [response.products count];
    NSArray *myproduct =response.products;
    NSLog(@"chanpin num=%d",[myproduct count]);
    //NSLog(@"BACK NUM =%d",index1);
...
IOSProduct *IOSiAP::iOSProductByIdentifier(std::string &identifier)
{
    ...
    for (iterator = iOSProducts.begin(); iterator != iOSProducts.end(); iterator++) {
        IOSProduct *iosProduct = *iterator;
        NSLog(@"iOSProductByIdentifier here");
...
void IOSiAP::requestProducts(std::vector <std::string> &productIdentifiers)
{
    ...
    NSLog(@"roductIdentifiers.size=%zu",productIdentifiers.size());
